...
function numericMeta(products, key) {
  const values = products.map((p) => p[key]).filter((v) => typeof v === "number" && Number.isFinite(v));
  if (!values.length) {
...
export async function loader({ request }) {
  try {
    ...
    if (!handle) {
      return new Response(
        JSON.stringify({ ok: false, error: "Missing handle parameter" }, null, 2),
    ...
    if (!collection) {
      return new Response(
        JSON.stringify({ ok: false, error: "Collection not found", handle }, null, 2),
